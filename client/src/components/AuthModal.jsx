...
const inputClass =
  "w-full h-12 px-4 bg-sand-100 border border-divider rounded-xl text-ink placeholder:text-sand-700 focus:outline-none focus:border-brand focus:ring-1 focus:ring-brand transition-all text-[15px]";

const submitClass =
  "w-full h-12 bg-brand hover:bg-brand-600 text-ground font-bold text-[15px] rounded-xl transition-all shadow-md shadow-soft mt-2";
  ...
  return (
    ...
    >
      <div
        ...
        aria-label={heading}
        tabIndex={-1}
        className="skhooler-panel-in relative w-full max-w-[420px] bg-surface rounded-2xl shadow-lg border border-divider p-8 flex flex-col items-center outline-none"
      >
        <button
          onClick={onClose}
          aria-label="Close"
          className="absolute top-4 right-4 text-sand-700 hover:text-ink transition-colors p-1"
        >
          <X className="w-5 h-5" />
        </button>

        <div className="flex items-center text-[36px] font-bold tracking-tighter text-ink mb-6">
          skhooler
        </div>

        {view === "login" && (
          <>
            <h2 className="text-xl font-bold text-ink mb-6">{heading}</h2>
            <form onSubmit={handleSubmit} className="w-full space-y-4">
              ...
              <div className="relative">
                <input
                  ...
                  className={inputClass}
                />
                <button type="button" onClick={() => setView("forgot")} className="absolute right-4 top-1/2 -translate-y-1/2 text-[13px] font-semibold text-sand-700 hover:text-ink">
                  Forgot?
                </button>
              </div>
            ...
            <div className="mt-8 text-[14px] text-sand-700 font-medium">
              Don&apos;t have an account? <button onClick={() => setView("signup")} className="text-brand-700 hover:underline font-semibold ml-1">Sign up</button>
            </div>
          </>
        )}
        ...
        {view === "signup" && (
          <>
            ...
            <div className="mt-8 text-[14px] text-sand-700 font-medium">
              Already have an account? <button onClick={() => setView("login")} className="text-brand-700 hover:underline font-semibold ml-1">Log in</button>
            </div>
          </>
        )}
        ...
        {view === "forgot" && (
          <>
            <h2 className="text-xl font-bold text-ink mb-2">{heading}</h2>
            {resetSent ? (
              <div className="text-center mt-4">
                ...
                <button onClick={() => setView("login")} className="mt-6 w-full h-12 bg-sand-100 hover:bg-sand-300 text-ink font-bold text-[15px] rounded-xl transition-colors">
                  Back to login
                </button>
              </div>
            ) : (
              <>
                <p className="text-[14px] text-sand-700 text-center mb-6">Enter your email address and we&apos;ll send you a link to reset your password.</p>
                <form onSubmit={handleSubmit} className="w-full space-y-4">
                  <input
                ...
                <button onClick={() => setView("login")} className="mt-6 text-[14px] text-sand-700 hover:text-ink font-medium transition-colors">
